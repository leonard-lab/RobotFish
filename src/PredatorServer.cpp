...
MT_Server::t_msg_def* MT_SM_Predator::getMessageDefs()
{
 ...
 * unknown) */
bool MT_SM_Predator::handleMessage(MT_Server::t_msg msg_code,
                                          wxSocketBase* sock)
{
...
void MT_SM_Predator::sendBlobInfo(wxSocketBase* sock)
{
    /* pull the data group from the tracker */
    MT_DataReport* dr_blob = m_pPredatorTracker->getDataReport(0);
    if(!dr_blob)
    {
        return;
    }
